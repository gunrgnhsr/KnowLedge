...
// Custom Rehype plugin to preserve data-line attached to math blocks
const rehypePreserveSync = () => (tree) => {
    visit(tree, 'element', (node, index, parent) => {
        // Find nodes created by remark-math (they have class 'math')
        if (node.properties && node.properties.className && Array.isArray(node.properties.className) && node.properties.className.includes('math')) {
            // Check if it has our data-line
            if (node.properties['data-line']) {
                const dataLine = node.properties['data-line'];

                // Remove data-line from the math node so it doesn't get copied/confused
                ...
                const wrapper = {
                    ...
                    tagName: 'span',
                    properties: {
                        className: ['math-sync-wrapper'],
                        'data-line': dataLine
                    },
                    children: [node]
                };

                // Replace the node with the wrapper in the parent
...
const processor = unified()
    ...
    .use(remarkRehype)
    .use(rehypePreserveSync) // Wrap BEFORE KaTeX
    .use(rehypeKatex);
